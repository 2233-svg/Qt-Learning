...
**一句话定位：** `QHBoxPlotModelMapper` 是 Qt 对象机制 中的类型，负责把这一机制中的数据、状态或资源交给其他 Qt 对象使用。

**模块背景：** Qt Charts 提供折线、柱状、饼图、散点图和坐标轴等数据可视化组件。

### 这是什么

`QHBoxPlotModelMapper` 是 Qt 对象机制 中的公开类型，作用是把这一机制里的一个职责封装成可组合的 API。
...
该属性包含作为第一个方框和胡须项数据源的模型行。
默认值为-1（映射无效）。

**如何使用：** 调用 `firstBoxSetRow()` 读取当前值；它不会修改应用状态。

### `firstColumn : int`

**作用与语义：**

该属性保存模型中包含箱线图序列中箱须图项的第一个值的列。
最小值和默认值为0。
...
该属性包含用作最后一个方框和胡须项目数据源的模型行。
默认值为-1（映射无效）。

**如何使用：** 调用 `lastBoxSetRow()` 读取当前值；它不会修改应用状态。
...
### `[explicit] QHBoxPlotModelMapper::QHBoxPlotModelMapper(QObject *parent = nullptr)`
...
**如何使用：** 这是变化通知信号。用 `connect()` 监听 `firstBoxSetRow` 的变化，不要把它当作普通函数主动调用。
...
**如何使用：** 这是变化通知信号。用 `connect()` 监听 `lastBoxSetRow` 的变化，不要把它当作普通函数主动调用。
...
**如何使用：** 调用 `setFirstBoxSetRow(...)` 修改 `firstBoxSetRow`；传入的新值会成为后续查询和相关界面行为所使用的值。
...
**如何使用：** 调用 `setLastBoxSetRow(...)` 修改 `lastBoxSetRow`；传入的新值会成为后续查询和相关界面行为所使用的值。
...
`QHBoxPlotModelMapper` 所属机制类型：Qt 对象机制。遇到重载时，优先对照参数类型、返回值和对象所有权；遇到布局、事件循环、线程、绘制或模型/视图问题时，要同时考虑本类与协作类之间的协议。
