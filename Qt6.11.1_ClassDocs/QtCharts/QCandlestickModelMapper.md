...
定义映射器使用的`QCandlestickSeries`对象。
注意：当序列设置为映射器时，所有序列数据都会被丢弃。当指定新序列时，旧序列会被断开（保持其数据）。
...
返回包含系列中集合未开值的模型行/列。默认值为-1（映射无效）。

### `[pure virtual] Qt::Orientation QCandlestickModelMapper::orientation() const`

**作用与语义：**

返回`QCandlestickModelMapper`访问模型时使用的方向。这决定了集合的连续值是从行（`Qt::Horizontal`）读取还是从列（`Qt::Vertical`）读取。
