...
**如何使用：** 调用 `xColumn()` 读取当前值；它不会修改应用状态。
...
**如何使用：** 调用 `yColumn()` 读取当前值；它不会修改应用状态。
...
**如何使用：** 这是变化通知信号。用 `connect()` 监听 `xColumn` 的变化，不要把它当作普通函数主动调用。
...
**如何使用：** 这是变化通知信号。用 `connect()` 监听 `yColumn` 的变化，不要把它当作普通函数主动调用。
...
**如何使用：** 调用 `setXColumn(...)` 修改 `xColumn`；传入的新值会成为后续查询和相关界面行为所使用的值。
...
**如何使用：** 调用 `setYColumn(...)` 修改 `yColumn`；传入的新值会成为后续查询和相关界面行为所使用的值。
