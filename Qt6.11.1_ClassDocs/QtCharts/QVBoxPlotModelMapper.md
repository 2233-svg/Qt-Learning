...
**一句话定位：** `QVBoxPlotModelMapper` 是 Qt 对象机制 中的类型，负责把这一机制中的数据、状态或资源交给其他 Qt 对象使用。

**模块背景：** Qt Charts 提供折线、柱状、饼图、散点图和坐标轴等数据可视化组件。

### 这是什么

`QVBoxPlotModelMapper` 是 Qt 对象机制 中的公开类型，作用是把这一机制里的一个职责封装成可组合的 API。
...
该属性保存作为第一个箱须图项数据源的模型列。
默认值为-1（无效映射）。

**如何使用：** 调用 `firstBoxSetColumn()` 读取当前值；它不会修改应用状态。

### `firstRow : int`

**作用与语义：**

该属性保留了模型中包含箱型图序列中箱状和胡须项首个值的行。
最低和默认值是0。
...
该属性保存作为最后一个箱须图项数据源的模型列。
默认值为-1（无效映射）。

**如何使用：** 调用 `lastBoxSetColumn()` 读取当前值；它不会修改应用状态。
...
该属性包含模型中映射为箱形图系列数据的行数。
最小值和默认值为-1（数字限制于模型中的列数）。
...
**如何使用：** 这是变化通知信号。用 `connect()` 监听 `firstBoxSetColumn` 的变化，不要把它当作普通函数主动调用。
...
**如何使用：** 这是变化通知信号。用 `connect()` 监听 `lastBoxSetColumn` 的变化，不要把它当作普通函数主动调用。
...
**如何使用：** 调用 `setFirstBoxSetColumn(...)` 修改 `firstBoxSetColumn`；传入的新值会成为后续查询和相关界面行为所使用的值。
...
**如何使用：** 调用 `setLastBoxSetColumn(...)` 修改 `lastBoxSetColumn`；传入的新值会成为后续查询和相关界面行为所使用的值。
...
`QVBoxPlotModelMapper` 所属机制类型：Qt 对象机制。遇到重载时，优先对照参数类型、返回值和对象所有权；遇到布局、事件循环、线程、绘制或模型/视图问题时，要同时考虑本类与协作类之间的协议。
