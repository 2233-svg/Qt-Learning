...
返回该对象所指的节点类型（即`TextNode`、`CDATASectionNode`、`CommentNode`或`CharacterDataNode`）。对于空节点，返回`CharacterDataNode`。

### `void QDomCharacterData::replaceData(unsigned long offset, unsigned long count, const QString &arg)`

**作用与语义：**

用字符串`arg`替换从位置`count` `offset`开始长度的子串。
