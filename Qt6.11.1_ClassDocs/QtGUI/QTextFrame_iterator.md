...
**一句话定位：** `QTextFrame::iterator` 是容器或范围的迭代器类型，用于按约定遍历元素；重点是有效期、可写性和失效规则。

**模块背景：** Qt GUI 负责窗口系统集成、绘制、颜色、字体、图像、输入事件和底层 GUI 资源。

### 这是什么

`QTextFrame::iterator` 是 迭代器与范围机制 中的公开类型，作用是把这一机制里的一个职责封装成可组合的 API。
...
如果当前项目是文本框中的最后一个，则返回`true`。

### `QTextBlock iterator::currentBlock() const`

**作用与语义：**

返回迭代器指向的当前块。如果迭代器指向子帧，返回的块无效。

### `QTextFrame *iterator::currentFrame() const`

**作用与语义：**

返回迭代器指向的当前帧，或返回`nullptr`，如果迭代器当前指向某个块。
...
如果迭代器与`other`迭代器不同，则返回真;否则返回`false`。
...
后缀`++`操作符（`i++`）将迭代器推进到文本帧中的下一个项，并返回一个迭代器到旧项。
...
后缀`--`操作符（`i--`）在当前帧中生成前一个项，并返回旧项的迭代器。

### `bool iterator::operator==(const QTextFrame::iterator &other) const`

**作用与语义：**

如果迭代器与`other`迭代器相同，则返回真;否则返回`false`。
...
`QTextFrame::iterator` 所属机制类型：迭代器与范围机制。遇到重载时，优先对照参数类型、返回值和对象所有权；遇到布局、事件循环、线程、绘制或模型/视图问题时，要同时考虑本类与协作类之间的协议。
