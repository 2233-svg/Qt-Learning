...
构建一个源自`device`的悬停事件对象。
`type`参数必须是`QEvent::HoverEnter`、`QEvent::HoverLeave`或`QEvent::HoverMove`。
`scenePos`是当前鼠标光标相对于接收窗口或场景的位置，`oldPos` 是其之前的位置，`globalPos` 是绝对坐标下的鼠标位置。`modifiers` 保持事件发生时所有键盘修改键的状态。

### `QPoint QHoverEvent::oldPos() const`

**作用与语义：**

返回鼠标光标相对于接收事件的小部件的上一个位置。如果没有之前的位置，oldPos() 将返回与`position()`相同的位置。
在事件`QEvent::HoverEnter`中，该位置始终为`QPoint`（-1， -1）。

### `QPointF QHoverEvent::oldPosF() const`

**作用与语义：**

返回鼠标光标相对于接收事件的小部件的前一个位置。如果没有之前的位置，oldPosF() 将返回与 `position()` 相同的位置。
在`QEvent::HoverEnter`事件中，该位置始终为`QPointF`（-1， -1）。
