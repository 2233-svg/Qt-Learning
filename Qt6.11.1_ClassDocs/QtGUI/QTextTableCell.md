...
`QTextTableCell` 是 Qt 类型机制 中的公开类型，作用是把这一机制里的一个职责封装成可组合的 API。
...
复制构造器。基于`other`单元创建新的QTextTableCell对象。
...
将单元格的字符格式设置为`format`。例如，这可以用来改变整个单元格的背景颜色：
注意，单元格的行或列跨度不能通过这个函数更改。你必须用`QTextTable::mergeCells`和`QTextTable::splitCell`来代替。
...
返回文档内部格式列表中tableCell格式的索引。

### `bool QTextTableCell::operator!=(const QTextTableCell &other) const`

**作用与语义：**

如果该单元格对象与`other`单元对象描述不同的单元格，返回`true`;否则返回`false`。
...
如果该单元格对象与`other`单元对象描述相同的单元格，返回`true`;否则返回`false`。
...
`QTextTableCell` 所属机制类型：Qt 类型机制。遇到重载时，优先对照参数类型、返回值和对象所有权；遇到布局、事件循环、线程、绘制或模型/视图问题时，要同时考虑本类与协作类之间的协议。
