...
**一句话定位：** `QTextObject` 是 Qt 对象机制 中的类型，负责把这一机制中的数据、状态或资源交给其他 Qt 对象使用。

**模块背景：** Qt GUI 负责窗口系统集成、绘制、颜色、字体、图像、输入事件和底层 GUI 资源。

### 这是什么

`QTextObject` 是 Qt 对象机制 中的公开类型，作用是把这一机制里的一个职责封装成可组合的 API。
...
- 继承自：QObject
- 直接派生类：QTextBlockGroup、QTextFrame
...
为给定`document`创建一个新的 QTextObject。
警告：此函数绝不应直接调用，只能从`QTextDocument::createObject()`调用。
...
返回该对象的对象索引。这可以与`QTextFormat::setObjectIndex()`一起使用。
...
`QTextObject` 所属机制类型：Qt 对象机制。遇到重载时，优先对照参数类型、返回值和对象所有权；遇到布局、事件循环、线程、绘制或模型/视图问题时，要同时考虑本类与协作类之间的协议。
