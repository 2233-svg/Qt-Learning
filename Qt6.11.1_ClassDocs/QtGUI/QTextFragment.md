...
`QTextFragment` 是 Qt 类型机制 中的公开类型，作用是把这一机制里的一个职责封装成可组合的 API。
...
将`other`文本片段的内容（文本和格式）复制到该文本片段。
...
返回文档内部字符格式列表中的索引，以索引文本片段的字符格式。

### `bool QTextFragment::contains(int position) const`

**作用与语义：**

如果文本片段包含文档中给定`position`的文本，返回`true`;否则返回`false`。

### `QList<QGlyphRun> QTextFragment::glyphRuns(int pos = -1, int len = -1) const`

**作用与语义：**

返回对应该文本片段中`len`字符的字形，从位置`pos`开始。字形的位置相对于`QTextBlock`布局的位置。
如果`pos`小于零，则默认从`QTextFragment`的起始点开始。如果`len`小于零，则默认为片段的长度。

### `bool QTextFragment::isValid() const`

**作用与语义：**

如果该片段有效（即在文档中有有效位置），返回`true`;否则返回`false`。
...
如果此文本片段与 `other` 文本片段不同（位于不同位置），则返回 `true`；否则返回 false。

### `bool QTextFragment::operator<(const QTextFragment &other) const`

**作用与语义：**

如果此文本片段出现在文档中比 `other` 文本片段更早的位置，则返回 `true`；否则返回 `false`。

### `QTextFragment &QTextFragment::operator=(const QTextFragment &other)`

**作用与语义：**

将`other`文本片段的内容（文本和格式）分配给该文本片段。

### `bool QTextFragment::operator==(const QTextFragment &other) const`

**作用与语义：**

如果此文本片段与 `other` 文本片段相同（在相同位置），则返回 `true`；否则返回 `false`。
...
`QTextFragment` 所属机制类型：Qt 类型机制。遇到重载时，优先对照参数类型、返回值和对象所有权；遇到布局、事件循环、线程、绘制或模型/视图问题时，要同时考虑本类与协作类之间的协议。
