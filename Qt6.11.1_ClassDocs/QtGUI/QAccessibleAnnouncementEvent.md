...
为`iface`创建新的QAccessibleAnnouncementEvent活动，以礼貌`QAccessible::AnnouncementPoliteness::Polite`请求公告的通知`message`。
`QAccessibleAnnouncementEvent::setPoliteness`可以用来调整礼貌。

### `[explicit] QAccessibleAnnouncementEvent::QAccessibleAnnouncementEvent(QObject *object, const QString &message)`

**作用与语义：**

为`object`创建新的QAccessibleAnnouncementEvent活动，礼貌地请求`message`公告，并以礼貌的态度`QAccessible::AnnouncementPoliteness::Polite`。
`QAccessibleAnnouncementEvent::setPoliteness`可以用来调整礼貌。
