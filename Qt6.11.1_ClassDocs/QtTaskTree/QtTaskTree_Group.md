...
**一句话定位：** `QtTaskTree::Group` 是并发执行或同步类型，负责任务、线程、future、promise 或共享资源的协调。

**模块背景：** 这是 Qt TaskTree 模块中的公开 C++ API，具体职责以类摘要和继承关系为准。

### 这是什么

`QtTaskTree::Group` 是 并发与任务机制 中的公开类型，作用是把这一机制里的一个职责封装成可组合的 API。
...
构造一个具有给定`children`列表的群。
当组的子项在编译时未知，但运行时后期，该构造器非常有用：
...
由`children`给出的`std::initializer_list`构造一个群。
当该组的所有子项在编译时都已知时，该构造器非常有用：
...
`QtTaskTree::Group` 所属机制类型：并发与任务机制。遇到重载时，优先对照参数类型、返回值和对象所有权；遇到布局、事件循环、线程、绘制或模型/视图问题时，要同时考虑本类与协作类之间的协议。
