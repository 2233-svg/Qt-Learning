...
构建映射任务树运行器。`Key`类型用于任务树映射。

### `QMappedTaskTreeRunner::~QMappedTaskTreeRunner()`

**作用与语义：**

摧毁映射的任务树运行器。所有运行中的任务树被删除。调用任何任务树的完成处理程序。

### `void QMappedTaskTreeRunner::cancel()`

**作用与语义：**

取消所有正在运行的任务树。使用 `DoneWith::Cancel` 调用任务树的完成处理程序。任务树取消的顺序是随机的。

### `void QMappedTaskTreeRunner::cancelKey(const Key &key)`

**作用与语义：**

取消可能正在运行的任务树，该任务树是使用 `key` 启动的。使用 `DoneWith::Cancel` 调用任务树的完成处理程序。

### `bool QMappedTaskTreeRunner::isKeyRunning(const Key &key) const`

**作用与语义：**

返回映射的任务树运行器当前是否正在执行以`key`启动的任务树。

### `bool QMappedTaskTreeRunner::isRunning() const`

**作用与语义：**

返回映射的任务树运行器是否正在执行至少一个任务树。

### `void QMappedTaskTreeRunner::reset()`

**作用与语义：**

重置所有正在运行的任务树。调用任何任务树的已完成处理程序。

### `void QMappedTaskTreeRunner::resetKey(const Key &key)`

**作用与语义：**

重置了用`key`启动的可能正在运行的任务树。调用的是没有任务树完成的处理程序。
...
无条件启动给定`key`的`recipe`，重置任何可能运行的任务树，并保持其他可能运行的任务树并行。调用在新任务树即将启动时进行`setupHandler`。调用在任务树完成时调用`doneHandler`。`doneHandler`根据传递的`callDone`调用。
