...
**一句话定位：** `QtTaskTree::QParallelTaskTreeRunner` 是并发执行或同步类型，负责任务、线程、future、promise 或共享资源的协调。

**模块背景：** 这是 Qt TaskTree 模块中的公开 C++ API，具体职责以类摘要和继承关系为准。

### 这是什么

`QtTaskTree::QParallelTaskTreeRunner` 是 并发与任务机制 中的公开类型，作用是把这一机制里的一个职责封装成可组合的 API。
...
销毁并行任务树运行器。所有正在运行的任务树被删除。调用任何任务树的已完成处理程序。
...
立即启动`recipe`，并保持其他可能正在运行的任务树并行运行。调用时`setupHandler`新的任务树即将启动。调用`doneHandler`任务树完成时。`doneHandler`根据传递的`callDone`调用。
...
`QtTaskTree::QParallelTaskTreeRunner` 所属机制类型：并发与任务机制。遇到重载时，优先对照参数类型、返回值和对象所有权；遇到布局、事件循环、线程、绘制或模型/视图问题时，要同时考虑本类与协作类之间的协议。
