...
**一句话定位：** `QDBusServer` 是 Qt 对象机制 中的类型，负责把这一机制中的数据、状态或资源交给其他 Qt 对象使用。

**模块背景：** 这是 Qt D-Bus 模块中的公开 C++ API，具体职责以类摘要和继承关系为准。

### 这是什么

`QDBusServer` 是 Qt 对象机制 中的公开类型，作用是把这一机制里的一个职责封装成可组合的 API。
...
构建一个带有给定`parent`的QDBusServer。服务器会监听`/tmp`（Unix系统）或绑定到localhost的TCP端口（其他地方）上的连接。

### `[explicit] QDBusServer::QDBusServer(const QString &address, QObject *parent = nullptr)`

**作用与语义：**

构建一个包含给定`address`和`parent`的QDBusServer。
...
当新客户端连接`connection`建立到服务器时，会发出该信号。

### `void QDBusServer::setAnonymousAuthenticationAllowed(bool value)`

**作用与语义：**

如果`value`设置为true，即使连接客户端未被认证为用户，入站连接仍可继续。
默认情况下，这个值为假。
...
`QDBusServer` 所属机制类型：Qt 对象机制。遇到重载时，优先对照参数类型、返回值和对象所有权；遇到布局、事件循环、线程、绘制或模型/视图问题时，要同时考虑本类与协作类之间的协议。
