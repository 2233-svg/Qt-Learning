...
将搜索查询小部件合并，只显示默认搜索字段。

### `void QHelpSearchQueryWidget::expandExtendedSearch()`

**作用与语义：**

扩展搜索查询小部件，使扩展搜索字段得以显示。

### `[signal] void QHelpSearchQueryWidget::search()`

**作用与语义：**

当用户调用搜索按钮时，该信号会发出。收到信号后，你可以向`QHelpSearchQueryWidget`请求搜索输入，并传递给`QHelpSearchEngine::search()`函数。

### `QString QHelpSearchQueryWidget::searchInput() const`

**作用与语义：**

返回一个搜索短语，可与 QHelpSearchEngine：：search（const `QString` &searchInput） 函数结合使用。

### `void QHelpSearchQueryWidget::setSearchInput(const QString &searchInput)`

**作用与语义：**

将`QHelpSearchQueryWidget`输入字段设置为`searchInput`指定的值。
注意：实际搜索需调用QHelpSearchEngine：：search（const `QString` & `searchInput`）函数。
