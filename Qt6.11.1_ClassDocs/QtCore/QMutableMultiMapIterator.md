...
返回上一个跳过的物品的密钥，该物品使用其中一个遍历函数（`next()`、`previous()`、`findNext()`、`findPrevious()`）。
在调用`next()`或`findNext()`后，`key()`等价于`peekPrevious()`。`key()`。在调用`previous()`或`findPrevious()`后，`key()`等价于`peekNext()`。`key()`。
...
返回下一个项目的引用，无需移动迭代器。
调用返回值的 `key()` 以获取该项的密钥，`value()` 调用以获取值。
在容器后部的迭代器上调用该函数会导致未定义的结果。
...
在调用`next()`或`findNext()`后，`value()`等价于`peekPrevious()`。`value()`。在调用`previous()`或`findPrevious()`后，`value()`等价于`peekNext()`。`value()`。
