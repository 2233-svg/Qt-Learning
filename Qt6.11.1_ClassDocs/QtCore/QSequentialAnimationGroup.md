...
**如何使用：** 调用 `currentAnimation()` 读取当前值；它不会修改应用状态。

### `QSequentialAnimationGroup::QSequentialAnimationGroup(QObject *parent = nullptr)`

**作用与语义：**

构造一个QSequentialAnimationGroup。`parent`传递给`QObject`的构造器。
...
为该动画组添加`msecs`暂停。暂停被视为一种特殊动画类型，因此`animationCount`会增加一个。
...
**如何使用：** 这是变化通知信号。用 `connect()` 监听 `currentAnimation` 的变化，不要把它当作普通函数主动调用。
...
**如何使用：** 调用 `bindableCurrentAnimation()` 取得 `currentAnimation` 的 `QBindable`，用于建立属性绑定；只读取当前值时直接使用普通 getter。
