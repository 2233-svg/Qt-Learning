...
销毁清理处理程序。该清理处理程序中的所有对象都会被删除。

### `QObject *QObjectCleanupHandler::add(QObject *object)`

**作用与语义：**

向该清理处理程序添加`object`，并返回指向该对象的指针。

### `void QObjectCleanupHandler::clear()`

**作用与语义：**

删除该清理处理程序中的所有对象。清理处理程序变空。

### `bool QObjectCleanupHandler::isEmpty() const`

**作用与语义：**

如果该清理处理程序为空或该清理处理程序中的所有对象均被销毁，返回`true`;否则返回 false。

### `void QObjectCleanupHandler::remove(QObject *object)`

**作用与语义：**

从该清理处理程序中移除`object`。该对象不会被销毁。
