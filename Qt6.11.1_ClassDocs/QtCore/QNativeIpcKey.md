...
该枚举描述了IPC对象的后端类型。有关密钥类型的详细信息，请参见本地IPC密钥文档。
- `QNativeIpcKey::Type::SystemV`：`0x51`;X/开放系统倡议（XSI）或系统V（SVr4）API
...
构造一个类型为`DefaultTypeForOs`的QNativeIpcKey对象，密钥为空。

### `QNativeIpcKey::QNativeIpcKey(const QString &key, QNativeIpcKey::Type type = DefaultTypeForOs)`

**作用与语义：**

构造一个`QNativeIpcKey`对象，包含本地密钥`key`（或在无参数的超载时空），用于类型`type`。
...
处理掉这个`QNativeIpcKey`物品。

### `[static] QNativeIpcKey QNativeIpcKey::fromString(const QString &text)`

**作用与语义：**

解析字符串形式`text`并返回相应的`QNativeIpcKey`。字符串表示有助于告知其他进程该进程创建的密钥及其应附加的密钥。
如果字符串无法解析，该函数返回一个`invalid`对象。

### `[noexcept] bool QNativeIpcKey::isEmpty() const`

**作用与语义：**

如果`nativeKey()`为空，则返回为真。

### `[noexcept] bool QNativeIpcKey::isValid() const`

**作用与语义：**

如果该对象包含有效的本地IPC密钥类型，则返回真。无效类型通常是由于未能用`fromString()`解析字符串表示。
该函数不检查该键串是否实际支持或是否适用于当前操作系统。

### `[static noexcept] QNativeIpcKey::Type QNativeIpcKey::legacyDefaultTypeForOs()`

**作用与语义：**

返回对应于 Qt 6.6 之前 `QSharedMemory` 和 `QSystemSemaphore` 使用的原生 IPC 键的`Type`。必须保持与 Qt 之前编译的任一类代码兼容的应用程序和库，可以使用该函数来判断其他应用程序可能使用的 IPC 类型。
注意，该函数依赖于Qt的构造具有相同的配置时间选项。
...
将本机IPC密钥和类型与`other`交换。该操作非常快速且从未失败。

### `QString QNativeIpcKey::toString() const`

**作用与语义：**

返回该对象的字符串表示。字符串表示有助于告知其他进程该进程创建的密钥及其应附加的密钥。
如果当前对象是`invalid`，该函数返回空字符串。
...
该常数表达式变量保留当前操作系统的默认本地IPC类型。Windows系统将`Type::Windows`，其他系统`Type::PosixRealtime`。注意，该常数与`QSharedMemory`和`QSystemSemaphore`在Qt 6.6之前大多数Unix系统默认使用的不同;更多信息请参见 `legacyDefaultTypeForOs()`。
...
将本地IPC密钥和类型`value1`与`value2`互换。该操作非常快速且从未失败。

### `[noexcept] bool operator!=(const QNativeIpcKey &lhs, const QNativeIpcKey &rhs)`

**作用与语义：**

如果`lhs`和`rhs`对象包含相同（或不同）的内容，则返回为真。
