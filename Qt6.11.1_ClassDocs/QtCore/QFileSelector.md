...
**一句话定位：** `QFileSelector` 是 文件、设备与流机制 中的类型，负责把这一机制中的数据、状态或资源交给其他 Qt 对象使用。

**模块背景：** Qt Core 提供对象模型、事件循环、容器、字符串、文件、线程、时间和元对象系统等基础能力。

### 这是什么

`QFileSelector` 是 文件、设备与流机制 中的公开类型，作用是把这一机制里的一个职责封装成可组合的 API。
...
创建一个 QFileSelector 实例。该实例将拥有与其他 QFileSelector 实例相同的静态选择器，但拥有一套额外的选择器。
如果提供，它将拥有给定的`QObject` `parent`。
...
返回该实例中通过程序添加的额外选择器列表。

### `QString QFileSelector::select(const QString &filePath) const`

**作用与语义：**

该函数基于运行时条件返回路径的选定版本。如果没有可选择的文件，返回原始的`filePath`。
如果原始文件不存在，原始`filePath`会返回。这意味着你必须有一个基础文件作为备份，不能只保留可选子目录中的文件。
请参见类别概述以了解选择算法。
...
这是对`QUrl`对象操作的 select 的便利版本。如果方案不是文件或 qrc，则立即返回 `filePath`。否则，选择会应用到路径`filePath`，返回一个`QUrl`，选中的路径和其他`QUrl`部分与 `filePath` 相同。
请参见类别概述以了解选择算法。
...
`QFileSelector` 所属机制类型：文件、设备与流机制。遇到重载时，优先对照参数类型、返回值和对象所有权；遇到布局、事件循环、线程、绘制或模型/视图问题时，要同时考虑本类与协作类之间的协议。
