...
返回该迭代器指向的映射值。如果容器未提供映射值（例如集合），返回无效`QVariant::Reference`。

### `int Iterator::operator*() const`

**作用与语义：**

返回当前项，转换为`QVariant::Reference`。如果有映射值，`QVariant::Reference`会解析为键值;如果没有，则解析为键值。

### `int Iterator::operator->() const`

**作用与语义：**

返回当前项，转换为`QVariant::Pointer`。如果有映射值，`QVariant::Pointer`解析为映射值，若无则解析为键值。
