...
该枚举用于控制位置数据的准确性。
- `QLocationPermission::Approximate`：`0`;请求一个大致的位置。
- `QLocationPermission::Precise`：`1`;要求提供一个精确的位置。
...
此枚举用于控制位置信息的可用性。
- `QLocationPermission::WhenInUse`: `0`; 位置信息仅在应用使用时可用。
- `QLocationPermission::Always`: `1`; 位置信息在任何时候都可用，包括应用在后台时。
