...
`QIterable` 是 Qt 类型机制 中的公开类型，作用是把这一机制里的一个职责封装成可组合的 API。
...
- 继承自：未在类页中列出
- 直接派生类：QAssociativeIterable、QSequentialIterable
...
返回是否可以向正向迭代容器。这对应于迭代器的std：：forward_iterator_tag迭代特征和容器的const_iterator。

### `bool QIterable::canInputIterate() const`

**作用与语义：**

返回容器是否有输入迭代器。这对应于迭代器的std：：input_iterator_tag迭代特性和容器的 const_iterator。

### `bool QIterable::canRandomAccessIterate() const`

**作用与语义：**

返回是否可以高效地使用迭代器跳过多个值。这对应于迭代器的std：：random_access_iterator_tag 迭代特征和容器的 const_iterator。

### `bool QIterable::canReverseIterate() const`

**作用与语义：**

返回是否可以反向迭代容器。这对应于容器const_iterator的std：：bidirectional_iterator_tag 迭代特征。
...
返回容器开头的`QConstIterator`。这可以在STL风格的迭代中使用。

### `QConstIterator<Container> QIterable::constEnd() const`

**作用与语义：**

返回容器末尾的Qterable：：QConstIterator。这可用于STL风格的迭代。

### `QIterator<Container> QIterable::mutableBegin()`

**作用与语义：**

返回容器开头的 `QIterator`。这可用于 STL 风格的迭代。

### `QIterator<Container> QIterable::mutableEnd()`

**作用与语义：**

返回容器末尾的QMetaSequence：：Iterable：：迭代器。这可用于STL风格的迭代。
...
返回容器中的数值。
注意：如果底层容器没有提供本地查询大小的方式，该方法将通过迭代器合成访问。该行为已被弃用，并将在未来 Qt 版本中移除。
...
`QIterable` 所属机制类型：Qt 类型机制。遇到重载时，优先对照参数类型、返回值和对象所有权；遇到布局、事件循环、线程、绘制或模型/视图问题时，要同时考虑本类与协作类之间的协议。
