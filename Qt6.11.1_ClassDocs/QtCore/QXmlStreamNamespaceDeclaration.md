...
创建包含 `prefix` 和 `namespaceUri` 的命名空间声明。
...
`QList`的同义词<`QXmlStreamNamespaceDeclaration`>。

### `[noexcept] bool operator!=(const QXmlStreamNamespaceDeclaration &lhs, const QXmlStreamNamespaceDeclaration &rhs)`

**作用与语义：**

将 `lhs` 命名空间声明与 `rhs` 进行比较，如果不相等则返回 `true`；否则返回 `false`。

### `[noexcept] bool operator==(const QXmlStreamNamespaceDeclaration &lhs, const QXmlStreamNamespaceDeclaration &rhs)`

**作用与语义：**

将 `lhs` 命名空间声明与 `rhs` 进行比较，如果它们相等则返回 `true`；否则返回 `false`。
