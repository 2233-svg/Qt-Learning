...
返回该迭代子指向的映射值，若无映射值则返回无效`QVariant`。

### `QVariant ConstIterator::operator*() const`

**作用与语义：**

返回当前项，转换为`QVariant`。返回的值是当前迭代器的映射值（如果有的话），否则是键。

### `int ConstIterator::operator->() const`

**作用与语义：**

返回当前项，转换为`QVariant::ConstPointer`。`QVariant::ConstPointer`会在当前迭代器（如果有）或键处的映射值解析。
