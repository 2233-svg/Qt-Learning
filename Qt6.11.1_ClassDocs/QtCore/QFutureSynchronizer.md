...
构建一个QFutureSynchronizer。

### `[explicit] QFutureSynchronizer::QFutureSynchronizer(QFuture<T> future)`

**作用与语义：**

构建一个QFutureSynchronizer，并通过调用`addFuture()`开始监视`future`。

### `QFutureSynchronizer::~QFutureSynchronizer()`

**作用与语义：**

调用`waitForFinished()`确保所有未来在销毁`QFutureSynchronizer`之前都已结束。

### `void QFutureSynchronizer::addFuture(QFuture<T> future)`

**作用与语义：**

这`future`被管理的未来清单增加了。

### `bool QFutureSynchronizer::cancelOnWait() const`

**作用与语义：**

如果启用了等待取消功能，返回`true`;否则返回 false。如果启用了等待取消功能，`waitForFinished()`函数会在等待未来结束前取消所有未来。

### `void QFutureSynchronizer::clearFutures()`

**作用与语义：**

移除所有管理型未来`QFutureSynchronizer`。
...
根据`enabled`参数启用或禁用等待取消功能。如果`enabled`为真，`waitForFinished()`函数会在等待未来结束前取消所有未来。

### `void QFutureSynchronizer::setFuture(QFuture<T> future)`

**作用与语义：**

将`future`设置为该`QFutureSynchronizer`唯一管理的未来。这是一个方便函数，调用`waitForFinished()`，然后`clearFutures()`，最后将`future`传递给`addFuture()`。

### `void QFutureSynchronizer::waitForFinished()`

**作用与语义：**

等待所有未来结束。如果`cancelOnWait()` `true`返回，每个未来在等待结束前都会被取消。
