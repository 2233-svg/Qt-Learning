...
**一句话定位：** `QFutureIterator` 是容器或范围的迭代器类型，用于按约定遍历元素；重点是有效期、可写性和失效规则。

**模块背景：** Qt Core 提供对象模型、事件循环、容器、字符串、文件、线程、时间和元对象系统等基础能力。

### 这是什么

`QFutureIterator` 是 迭代器与范围机制 中的公开类型，作用是把这一机制里的一个职责封装成可组合的 API。
...
构建一个遍历`future`的迭代器。迭代器设在结果列表的最前面（第一个结果之前）。
...
调用后，如果找到`value`，迭代器会被放置在匹配结果之后;否则，迭代器会被放置在结果列表的最后。

### `bool QFutureIterator::findPrevious(const T &value)`

**作用与语义：**

从当前迭代器位置向后搜索`value`。如果找到`value`，返回`true`;否则返回`false`。
调用后，如果找到`value`，迭代器会被放置在匹配结果的正前方;否则，迭代器会被放置在结果列表的最前面。

### `bool QFutureIterator::hasNext() const`

**作用与语义：**

如果迭代器前方至少有一个结果，例如迭代器不在结果列表的最后，返回`true`;否则返回false。

### `bool QFutureIterator::hasPrevious() const`

**作用与语义：**

如果迭代器前方至少有一个结果，例如迭代器不在结果列表的最前面，返回`true`;否则返回 false。
...
返回之前的结果，无需移动迭代器。
在结果列表前端的迭代器上调用该函数会导致结果未定义。

### `const T &QFutureIterator::previous()`

**作用与语义：**

返回之前的结果，并将迭代器向后移动一个位置。
在结果列表前端的迭代器上调用该函数会导致结果未定义。
...
将迭代器移到结果列表的最前面（第一个结果之前）。

### `QFutureIterator<T> &QFutureIterator::operator=(const QFuture<T> &future)`

**作用与语义：**

使迭代器对`future`进行操作。迭代器被设置为位于结果列表的最前面（第一个结果之前）。
...
`QFutureIterator` 所属机制类型：迭代器与范围机制。遇到重载时，优先对照参数类型、返回值和对象所有权；遇到布局、事件循环、线程、绘制或模型/视图问题时，要同时考虑本类与协作类之间的协议。
