...
为给定`role`构造一个QModelRoleData对象。

### `[noexcept] void QModelRoleData::clearData()`

**作用与语义：**

清除此对象所持有的数据。请注意，role 保持不变；仅数据被清除。
...
将该对象所持有的数据设置为`value`。`value`必须是可以存储在`QVariant`中的数据类型。
注意：该功能只有在`noexcept(m_data.setValue(std::forward<T>(value)))` `true`时才会生效。
