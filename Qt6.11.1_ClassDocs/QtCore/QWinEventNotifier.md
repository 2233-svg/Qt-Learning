...
构造一个带有给定`parent`的事件通知符。

### `[explicit] QWinEventNotifier::QWinEventNotifier(QWinEventNotifier::HANDLE hEvent, QObject *parent = nullptr)`

**作用与语义：**

构造一个带有给定`parent`的事件通知器。它启用该通知器，并监视事件`hEvent`。
通知器默认启用，即每当对应事件被提示时发出`activated()`信号。不过，通常建议明确启用或禁用事件通知器。
...
当事件通知器被启用且对应的HANDLE被信号时，该信号就会发出。
事件状态在过程中不会被修改，所以如果是手动重置事件，你需要在收到通知后重置它。
该对象以`hEvent`参数传递。
...
注册HANDLE `hEvent`。旧的HANDLE会自动取消注册。
