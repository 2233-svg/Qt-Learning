...
`Qt::partial_ordering` 是 Qt 类型机制 中的公开类型，作用是把这一机制里的一个职责封装成可组合的 API。
...
利用以下规则从`stdorder`构造一个`Qt::partial_ordering`对象：
- std：:p artial_ordering：：less 转换为 `less`。
- std：:p artial_ordering：：等效 转换为 `equivalent`。
- STD：:p Artial_Ordering：：greater 转换为 `greater`。
- std：:p artial_ordering：：unordered 转换为 `unordered`

### `[constexpr noexcept] partial_ordering::operator std::partial_ordering() const`

**作用与语义：**

利用以下规则将该`Qt::partial_ordering`值转换为 std：:p artial_ordering 对象：
- `less` 转换为 std：:p artial_ordering：：less。
- `equivalent` 转换为 std：:p artial_ordering：：equivalent。
- `greater` 转换为 std：:p artial_ordering：：greater。
- `unordered` 转换为 std：:p artial_ordering：：unordered。
...
`Qt::partial_ordering` 所属机制类型：Qt 类型机制。遇到重载时，优先对照参数类型、返回值和对象所有权；遇到布局、事件循环、线程、绘制或模型/视图问题时，要同时考虑本类与协作类之间的协议。
