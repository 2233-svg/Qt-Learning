...
构建一个QRestAccessManager对象，将`parent`设为父对象，`manager`作为用于通信的底层`QNetworkAccessManager`。

### `[override virtual noexcept] QRestAccessManager::~QRestAccessManager()`

**作用与语义：**

摧毁`QRestAccessManager`物体。
...
根据`request`发布`HTTP DELETE`。
可选的`callback`和`context`对象可用于处理请求完成，如下所示：
...
根据`request`发出`HTTP GET`。
可选的 `callback` 和 `context` 对象可用于处理请求完成，如下所示：
或者，也可以使用返回的`QNetworkReply`*对象的信号。欲了解更多信息，请参见“发出网络请求和处理回复”。
...
根据`request`和提供的`data` `HTTP GET`。
可选的`callback`和`context`对象可以用于处理请求完成，如下图所示：
或者，也可以使用返回的`QNetworkReply`*对象的信号。更多信息请参见“发出网络请求与处理回复”。
...
根据`request`发布`HTTP HEAD`。
可选的 `callback` 和 `context` 对象可用于处理请求完成，如下图所示：
或者，也可以使用返回的`QNetworkReply`*对象的信号。更多信息请参见“发出网络请求与处理回复”。
`head()`请求不支持提供数据。
...
根据`request`发布`HTTP PATCH`。
可选的`callback`和`context`对象可用于处理请求完成，如下所示：
或者，也可以使用返回的`QNetworkReply`*对象的信号。更多信息请参见“发出网络请求和处理回复”。
`patch()`方法总是需要`data`参数。支持以下数据类型：
- `QByteArray`
- `QJsonDocument` *）
- `QVariantMap` **）
- `QIODevice`*
*） 以`QJsonDocument::Compact`格式发送，且`Content-Type`头设置为`application/json`，如果`Content-Type`头未设置**） `QVariantMap`被转换为并作为`QJsonObject`处理。
...
基于`request`发布`HTTP POST`。
可选的`callback`和`context`对象可用于处理请求完成，如下所示：
或者，也可以使用返回的`QNetworkReply`*对象的信号。更多信息请参见“发出网络请求和处理回复”。
`post()`方法始终需要`data`参数。支持以下数据类型：
- `QByteArray`
- `QJsonDocument` *）
- `QVariantMap` **）
- `QHttpMultiPart`*
- `QIODevice`*
*） 以`QJsonDocument::Compact`格式发送，且如果`Content-Type`首部未设置，`Content-Type`首部设为`application/json` **） `QVariantMap`被转换为并作为`QJsonObject`处理。
...
根据`request`发布`HTTP PUT`。
可选的`callback`和`context`对象可用于处理请求完成，如下所示：
或者，也可以使用返回的`QNetworkReply`*对象的信号。更多信息请参见“发出网络请求和处理回复”。
`put()`方法总是需要`data`参数。支持以下数据类型：
- `QByteArray`
- `QJsonDocument` *）
- `QVariantMap` **）
- `QHttpMultiPart`*
- `QIODevice`*
*） 以`QJsonDocument::Compact`格式发送，且`Content-Type`头设置为`application/json`，如果`Content-Type`首部未设置 **） `QVariantMap`被转换为并作为`QJsonObject`处理。
...
问题`request`基于基于HTTP Request的自定义`method`和提供的`data`。
可选的`callback`和`context`对象可用于处理请求完成，如下图所示：
或者，也可以使用返回的`QNetworkReply`*对象的信号。更多信息请参见“发出网络请求和处理回复”。
