...
**一句话定位：** `QHttpPart` 是 Qt Network 的“HttpPart”类型，负责描述请求/地址/连接状态或承载异步网络数据。

**模块背景：** Qt Network 提供 TCP/UDP、HTTP、代理、DNS、SSL 和网络请求等异步网络能力。

### 这是什么

`QHttpPart` 是 异步网络机制 中的公开类型，作用是把这一机制里的一个职责封装成可组合的 API。
...
将该 MIME 部分的正体设置为 `body`。除非设备通过 `setBodyDevice()` 设置，否则使用该方法的正体。对于大量数据（例如图像），使用`setBodyDevice()`，它不会在内部复制数据。

### `void QHttpPart::setBodyDevice(QIODevice *device)`

**作用与语义：**

设置设备读取内容到 `device`。对于大量数据，这种方法应优先于 `setBody()`，因为使用该方法时内容不会被复制，而是直接从设备读取。`device`必须是开放且可读的。`QHttpPart` 不拥有 `device`，即设备必须关闭并在必要时销毁。如果 `device` 是顺序的（例如套接字，但不是文件`QNetworkAccessManager::post()`），应在 `device` 发出 finished() 后调用 。如需解除设备并通过`setBody()`使用数据集，请使用“setBodyDevice（0）”。

### `void QHttpPart::setHeader(QNetworkRequest::KnownHeaders header, const QVariant &value)`

**作用与语义：**

将已知头部`header`值设置为`value`，覆盖之前设置的任何头部。
...
将HTTP部分替换为`other`。这个操作非常快，从未失败过。

### `bool QHttpPart::operator!=(const QHttpPart &other) const`

**作用与语义：**

如果该对象与`other`不同，返回`true`。
...
如果该对象与`other`相同（即它们的头部和正体相同），返回`true`。

### `[since 6.8] QDebug operator<<(QDebug debug, const QHttpPart &part)`

**作用与语义：**

将`part`写入`debug`对象以供调试。除非设备被设置，否则会显示主体大小。
...
`QHttpPart` 所属机制类型：异步网络机制。遇到重载时，优先对照参数类型、返回值和对象所有权；遇到布局、事件循环、线程、绘制或模型/视图问题时，要同时考虑本类与协作类之间的协议。
