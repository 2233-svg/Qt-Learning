...
**一句话定位：** TCP 客户端套接字，负责建立连接、读写字节流和处理断开/错误。

**模块背景：** Qt Network 提供 TCP/UDP、HTTP、代理、DNS、SSL 和网络请求等异步网络能力。

### 这是什么

`QTcpSocket`：TCP 客户端套接字，负责建立连接、读写字节流和处理断开/错误。
...
在状态`UnconnectedState`创建QTcpSocket对象。
...
`QTcpSocket` 所属机制类型：异步网络机制。遇到重载时，优先对照参数类型、返回值和对象所有权；遇到布局、事件循环、线程、绘制或模型/视图问题时，要同时考虑本类与协作类之间的协议。
