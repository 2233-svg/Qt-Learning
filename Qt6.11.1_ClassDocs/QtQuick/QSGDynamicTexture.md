...
**一句话定位：** `QSGDynamicTexture` 是 Qt Quick 场景图渲染类型，负责节点、材质、纹理、几何或渲染状态。

**模块背景：** Qt Quick 面向 QML/场景图界面，提供可视项、动画、输入和高性能界面基础设施。

### 这是什么

`QSGDynamicTexture` 是 Qt Quick/QML 体系中的公开类型，连接 C++ 对象、QML 属性绑定和场景图渲染。
...
调用此函数以显式更新动态纹理。如果纹理因更新而改变，函数返回 true；否则返回 false。注意：此函数通常在 `QQuickItem::updatePaintNode()` 或 `QSGNode::preprocess()` 中调用，即在场景图的 `synchronization` 或 `node preprocessing` 阶段调用。不推荐在其他时间调用，这可能导致意外行为。
...
`QSGDynamicTexture` 所属机制类型：Qt Quick 场景图机制。遇到重载时，优先对照参数类型、返回值和对象所有权；遇到布局、事件循环、线程、绘制或模型/视图问题时，要同时考虑本类与协作类之间的协议。
