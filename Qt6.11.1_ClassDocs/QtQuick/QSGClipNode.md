...
**一句话定位：** `QSGClipNode` 是 Qt Quick 场景图渲染类型，负责节点、材质、纹理、几何或渲染状态。

**模块背景：** Qt Quick 面向 QML/场景图界面，提供可视项、动画、输入和高性能界面基础设施。

### 这是什么

`QSGClipNode` 是 Qt Quick/QML 体系中的公开类型，连接 C++ 对象、QML 属性绑定和场景图渲染。
...
删除了这个`QSGClipNode`。
如果`QSGNode::OwnsGeometry`设置了标志，几何体也会被删除。
...
将该剪辑节点的 clip rect 设置为 `rect`。
当矩形剪辑与`setIsRectangular`结合时，渲染器在某些情况下可能会采用更优的剪辑方法。

### `void QSGClipNode::setIsRectangular(bool rectHint)`

**作用与语义：**

将该剪辑节点是否包含矩形剪辑设置为`rectHint`。
这是一个优化提示，意味着渲染器可以使用剪刀式代替模板，这样速度快得多。
当该提示设置并适用时，剪辑区域将由`clipRect()`生成，而非`geometry()`。
默认情况下，该房产是`false`。
...
`QSGClipNode` 所属机制类型：Qt Quick 场景图机制。遇到重载时，优先对照参数类型、返回值和对象所有权；遇到布局、事件循环、线程、绘制或模型/视图问题时，要同时考虑本类与协作类之间的协议。
