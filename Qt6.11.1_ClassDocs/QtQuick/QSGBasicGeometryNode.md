...
删除了这个`QSGBasicGeometryNode`。
如果节点设置了`QSGNode::OwnsGeometry`标志，它也会删除它所指向的几何对象。该标志默认不被设置。
...
将该节点的几何形状设置为`geometry`。
如果节点设置了`QSGNode::OwnsGeometry`标志，它也会删除它指向的几何对象。该标志默认不会被设置。
如果几何体在未再次调用 setGeometry() 的情况下更改，用户还必须使用 `QSGNode::markDirty()` 标记该几何体为脏几何体。
