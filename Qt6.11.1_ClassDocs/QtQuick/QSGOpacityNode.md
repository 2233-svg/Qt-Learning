...
**一句话定位：** `QSGOpacityNode` 是 Qt Quick 场景图渲染类型，负责节点、材质、纹理、几何或渲染状态。

**模块背景：** Qt Quick 面向 QML/场景图界面，提供可视项、动画、输入和高性能界面基础设施。

### 这是什么

`QSGOpacityNode` 是 Qt Quick/QML 体系中的公开类型，连接 C++ 对象、QML 属性绑定和场景图渲染。
...
构建一个默认不透明度为1的不透明度节点。
不透明度在场景图中向下累积，因此一个节点上方有两个QSGOpacityNode实例，且不透明度均为0.5，其有效不透明度为0.25。
节点默认不透明度为1。
...
将该节点的不透明度设置为`opacity`。
渲染图之前，渲染器会对子树进行更新，将不透明度传播到其子树。
该值将被限制在0到1的范围内。
...
`QSGOpacityNode` 所属机制类型：Qt Quick 场景图机制。遇到重载时，优先对照参数类型、返回值和对象所有权；遇到布局、事件循环、线程、绘制或模型/视图问题时，要同时考虑本类与协作类之间的协议。
