...
**一句话定位：** `QSGSimpleRectNode` 是 Qt Quick 场景图渲染类型，负责节点、材质、纹理、几何或渲染状态。

**模块背景：** Qt Quick 面向 QML/场景图界面，提供可视项、动画、输入和高性能界面基础设施。

### 这是什么

`QSGSimpleRectNode` 是 Qt Quick/QML 体系中的公开类型，连接 C++ 对象、QML 属性绑定和场景图渲染。
...
构建一个带有空矩形和白色的QSGSimpleRectNode实例。

### `QSGSimpleRectNode::QSGSimpleRectNode(const QRectF &rect, const QColor &color)`

**作用与语义：**

构建一个QSGSimpleRectNode实例，其跨度为`rect`，颜色`color`。
...
将该矩形节点的矩形设置为从（`x`， `y`）开始，宽度为`w`，高度为`h`。
...
`QSGSimpleRectNode` 所属机制类型：Qt Quick 场景图机制。遇到重载时，优先对照参数类型、返回值和对象所有权；遇到布局、事件循环、线程、绘制或模型/视图问题时，要同时考虑本类与协作类之间的协议。
