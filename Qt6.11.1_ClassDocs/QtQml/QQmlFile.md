...
**中文解读：** 这是静态工具 API `isLocalFile`，不依赖某个实例的运行时状态。适合直接完成转换、查找、工厂创建或一次性操作；调用前仍要检查返回值和错误输出。
...
**中文解读：** 这是静态工具 API `urlToLocalFileOrQrc`，不依赖某个实例的运行时状态。适合直接完成转换、查找、工厂创建或一次性操作；调用前仍要检查返回值和错误输出。
