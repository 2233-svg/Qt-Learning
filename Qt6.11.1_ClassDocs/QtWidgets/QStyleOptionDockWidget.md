...
- `QStyleOptionDockWidget::Type`：`SO_DockWidget`;提供样式选项类型（本类`SO_DockWidget`）。
...
- `QStyleOptionDockWidget::Version`：`1`;2
该版本被`QStyleOption`子类用来实现扩展而不破坏兼容性。如果你用`qstyleoption_cast()`，通常不需要检查。

### `QStyleOptionDockWidget::QStyleOptionDockWidget()`

**作用与语义：**

构建一个 QStyleOptionDockWidget，将成员变量初始化为默认值。
