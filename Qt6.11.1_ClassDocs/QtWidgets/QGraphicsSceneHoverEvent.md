...
返回最后记录的场景坐标，即视图接收到的上一个鼠标或悬停事件的场景坐标，该事件创建了场景坐标中的鼠标光标位置。

### `QPoint QGraphicsSceneHoverEvent::lastScreenPos() const`

**作用与语义：**

返回屏幕上坐标中最后记录的鼠标光标位置。最后记录的位置是创建事件的视图接收到的上一个鼠标或悬停事件的位置。
