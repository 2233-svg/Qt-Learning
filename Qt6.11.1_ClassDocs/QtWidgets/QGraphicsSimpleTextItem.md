...
构建一个 QGraphicsSimpleTextItem，使用 `text` 作为默认明文。
`parent`传递给`QGraphicsItem`的构造函数。
...
将用于绘制物品文本的字体设置为`font`。

### `void QGraphicsSimpleTextItem::setText(const QString &text)`

**作用与语义：**

将物品文本设置为`text`。文本将以纯文本形式显示。换行字符（'\n'）以及类型为`QChar::LineSeparator`的字符会导致物品将文本拆分为多行。
