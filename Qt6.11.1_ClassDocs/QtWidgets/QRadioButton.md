...
用给定的 `parent` 构建一个单选按钮，但没有文本或像素图。
`parent` 参数传递给 `QAbstractButton` 构造器。

### `[explicit] QRadioButton::QRadioButton(const QString &text, QWidget *parent = nullptr)`

**作用与语义：**

用给定的`parent`和`text`字符串构造一个无线电按钮。
`parent`论元传递给`QAbstractButton`构造器。
...
用这个`QRadioButton`的值初始化`option`。这种方法对需要`QStyleOptionButton`但不想自己填写所有信息的子类很有用。
