...
这个枚举指定了`QRubberBand`应有的形状。这是一个向下传递给样式系统的绘图提示，每个`QStyle`都可以进行解释。
- `QRubberBand::Line`：`0`;`QRubberBand`可以表示垂直线或水平线。几何形状仍以`rect()`形式表示，线条在大多数样式中填充给定几何形状。
- `QRubberBand::Rectangle`：`1`;`QRubberBand`可以表示矩形。有些风格会将其理解为填充（通常是半透明）矩形或矩形轮廓。
...
构造一条带有母体`p`的形状橡皮筋`s`。
默认情况下，矩形橡皮筋（`s`为`Rectangle`）会使用遮罩，因此矩形的边缘是唯一可见的部分。某些样式（例如原生macOS）会改变这一点，调用`QWidget::setWindowOpacity()`来生成半透明填充的选区矩形。
...
用这个`QRubberBand`的值初始化`option`。这种方法适用于子类需要`QStyleOptionRubberBand`但不想自己填满所有信息时。
...
将橡皮筋的几何形状设置为`rect`，具体由其父控件的坐标系指定。

### `void QRubberBand::setGeometry(int x, int y, int width, int height)`

**作用与语义：**

将橡皮筋的几何形状设置为左上角位于该点（`x`， `y`）的矩形，尺寸由`width`和`height`指定。几何形状在父控件的坐标系中指定。
