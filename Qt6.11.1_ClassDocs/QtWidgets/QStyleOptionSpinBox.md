...
- `QStyleOptionSpinBox::Type`：`SO_SpinBox`;提供的样式类型（本类别`SO_SpinBox`）。
...
构建一个QStyleOptionSpinBox，将成员变量初始化为默认值。
...
该变量包含旋转盒中需要绘制的按钮符号类型。
默认数值是`QAbstractSpinBox::UpDownArrows`经典风格的精细小箭头。
...
该变量显示旋转盒中启用的按钮。
默认值是`QAbstractSpinBox::StepNone`。
